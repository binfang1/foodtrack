...
export default function Login({loggedIn, setLoggedIn, accounts}) {
    ...
    const [user, setUser] = useState("Test account");
    ...
    return (
        <div className="w-[300px] h-[300px] bg-white drop-shadow-md rounded-xl border-solid border-3 border-[#D9D9D9] m-auto">
            <form onSubmit={checkLogin} className="flex flex-col m-auto gap-[32px] text-center">
                <h1 className="text-[32px]">Login</h1>
                <label>Username:
                    <input onChange={getUser} value = "Test account" className = "border-gray-500 border-2 pl-[2px] pr-[2px] text-black" type="text" required/>
                </label>
                <label>Password:
                    <input onChange={getPass} value = "Test password" className = "border-gray-500 border-2 pl-[2px] pr-[2px] text-black" type="text" required/>
                </label>
